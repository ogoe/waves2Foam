...
void stokesFifth::setCoefficients()
{
    ...
    A31_ = (-4.0 - 20.0*S + 10.0*pow(S,2.0) - 13.0*pow(S,3.0))/(8.0*sinh(K_*h_)*pow(1.0 - S,3.0));
    ...
    C4_ = sqrt(tanh(K_*h_))*(4.0 + 32.0*S - 116.0*pow(S,2.0) - 400.0*pow(S,3.0) - 71.0*pow(S,4.0) + 146.0*pow(S,5.0))/(32.0*pow(1.0 - S,5.0));

    // D-coefficients
    D2_ = - sqrt(1.0/tanh(K_*h_))/2.0;
    D4_ =   sqrt(1.0/tanh(K_*h_))*(2.0 + 4.0*S + pow(S,2.0) + 2.0*pow(S,3.0))/(8.0*pow(1.0 - S,3.0));
...
) const
{
    ...
    scalar eta = (
                  ...
                  + pow(eps,4.0)*(B42_*Foam::cos(2.0*arg) + B44_*Foam::cos(4.0*arg))
                  + pow(eps,5.0)*( - (B53_ + B55_)*Foam::cos(arg) + B53_*Foam::cos(3.0*arg) + B55_*Foam::cos(5.0*arg))
