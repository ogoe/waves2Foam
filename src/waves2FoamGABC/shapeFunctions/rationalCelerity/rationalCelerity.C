...
defineTypeNameAndDebug(rationalCelerity, 0);
...
scalar rationalCelerity::func(const scalar& z) const
{
    return (a0_ + a1_*z + a2_*Foam::pow(z, 2.0))/
           (1   + a3_*z + a4_*Foam::pow(z, 2.0));
}


scalarField rationalCelerity::func(const scalarField& z) const
{
...
rationalCelerity::rationalCelerity
(
    const fvMesh& mesh,
...
:
    ...
    a4_(readScalar(dict.subDict(dictName()).lookup("a4")))
{
}
...
tmp<scalarField> rationalCelerity::rhoCelerity
(
	const fvPatch& p,
...
void rationalCelerity::rhoCelerity
(
    const fvPatch& p,
    const scalarField& rho,
    const scalarField& alpha,
